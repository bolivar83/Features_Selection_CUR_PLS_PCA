Exp<-function(DataExp){
  ...
  met.class<-"lda"  #  knn
  met.val<-"cv"     # {"loo","bt"}
  met.comp<-"pls"   # {"pca","NULL"}
  #Almacenamieto
  D<-data.frame(cbind(c2=0,c3=0,c4=0,c5=rep(0,nrun)))
  Frec<-data.frame(cbind(c2=0,c3=0,c4=0,c5=rep(0,n[1])))
  Best<-data.frame(cbind(v1=0,v2=0,v3=0,v4=0,v5=0,E=0,run=0,c=rep(0,1)))
  
  for(i in 2:k.cur){
    for(j in 1:nrun){
      OutPut<-Main.AHSA(DATA,Y,k.cur,k.comp=i,w,IterMax=200,met.cur=2,met.comp,met.class,met.val)
      D[j,i-1]<-OutPut$EBCV
      for(k in 1:length(VarSelect)){
        v<-VarSelect[k]
        Frec[v,i-1]<-Frec[v,i-1]+1                                                      # cáculo de la frecuencia de aparición del atributo
      }
      if(OutPut$EBCV>BestEBCV){                                   # actualizar la mejor selección                                                                                                                         
        BestVAR<-OutPut$BestVar
        BestEBCV<-OutPut$EBCV
        BestRun<-c(i,j)
        Best[1,1:5]<-BestVAR
        Best[1,6]<- BestEBCV
        Best[1,7:8]<-BestRun
      }
      write.matrix(D,file="E_lda_cv_pls_5a.csv",sep=";")
      write.matrix(Frec,file="F_lda_cv_pls_5a.csv",sep=";")
      write.matrix(Best,file="B_lda_cv_pls_5a.csv",sep=";")
    }
  }
}
##############################################################################################################
Main.AHSA<-function(DATA,Y,k.cur,k.comp,w,IterMax,met.cur,met.comp,met.class,met.val){
  ...
  return(list(BestVar=BestVar,NumBestVar=NumBestVar,EBCV=BestFitnessBCV,HH=HH))
