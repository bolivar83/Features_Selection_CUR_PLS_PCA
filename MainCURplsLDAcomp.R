Main.CUR.PLS.LDA.comp<-function(DATA,Y,VarName,k.cur,k.pls,w,IterMax,met.cur,Mes){
  ...
  DATA<-pca$XScale
  A<-BuildModelCUR(DATA,Y,pca$xSvd,k.cur,k.pls,met.cur,Mes)
  ...
  for(i in 1:length(VarSelect)){
    ...
    Hfimp[j]<- Hfimp[j]+(w*((Hfvar[j]/IterMax)+(HEbcv[j])/(Hfvar[j]))+(1-w)*(HIvar[j]/Hfvar[j])) # cálculo del factor de permanencia (FALTA ->  w3*HIvar[j])
  }
  ...
  return(list(BestVar=BestVar,BestVarNum=BestVarNum,EBCV=BestFitnessBCV,HH=HH))
  # Salidas
...
#----------------------------------------------------------------------------------------------------------------
BuildModelCUR<-function(DATA,Y,xSvd,k.cur,k.pls,met.cur,Mes)
{
  ...
  while(Stop==0){
    CURResults <- CUR(Dmatriz, c= k.cur, r= dim(Dmatriz)[1], k = k.cur, sv = xSvd ,method,alpha, weighted = FALSE, beta = 4,matrix.return = TRUE) 
    SVarCUR <- CURResults@C.index              # variables Seleccionados                  
    if(length(SVarCUR)>1){
      Stop<-1
    }
  }
    Aux<-PLS.LDA.errorBCV(Dmatriz,Y,SVarCUR,k.cur,k.pls,Mes)
...
PLS.LDA.errorBCV<-function(Dmatriz,Y,SVarCUR,k.cur,k.pls,Mes){
  # Entrada
  ...
  # met.pls=1 Calcula las componetes PLS de SVarCUR para calcular el modelo descriminante 
    ...
    XX<-Dmatriz[,SVarCUR]
    nc<-length(SVarCUR)
    pls.out <- pls.regression(Xtrain =XX, Ytrain = Y, ncomp =nc)
    #pls.class <- pls.regression(Xtrain =XX, Ytrain = Y, ncomp = k.pls)
    #Xclass<- pls.class$T
    X<- pls.out$T
    W <- pls.out$R
    YX <- data.frame(Y,X[,1:k.pls])
    RdYTm<-rep(0,nc)
    RdYTh<-rep(0,nc)
    VIP<-rep(0,nc)
   for (i in 1:nc){
